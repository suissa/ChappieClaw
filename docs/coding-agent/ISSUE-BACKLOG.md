...
GitHub Issues are currently disabled on `suissa/ChappieClaw`. The GitHub API returns HTTP 410 for issue creation. This file preserves the intended issue split so it can be converted 1:1 when Issues are enabled.

1. **feat(coding): explicit Coding Agent lifecycle with hypothesis-before-mutation**
...
2. **feat(skills): consume AllasCode atomic Action SKILLs as canonical coding knowledge**
   - Support semantics/authoring/healing projections, canonical labels, retrieval metadata, relations, and authority.

3. **feat(skills): semantic Skill Resolver and bounded context composition**
...
4. **feat(tools): coding-specific search/build/test/lint/format/diagnostics tools**
   - Add language-aware adapters for Zig, Rust, Go, and TypeScript.

5. **feat(safety): hashed mutation, workspace policy, and change guards**
   - Prefer hashed edits, reject stale state, constrain paths/commands, define revert boundary.

6. **feat(agents): CodeManager, CodeHealer, and independent HealingVerifier profiles**
   - Separate contexts and tool authority. Verifier must not inherit healer reasoning.

7. **feat(healing): successful verified fixes become candidate Healing SKILLs**
   - Candidate -> validate -> promote. Failed/inconclusive results cannot become trusted.

8. **feat(acp): coding sessions through Agent Client Protocol**
   - Bind workspace and expose phase/diagnostic/verification state without bypassing policy.

9. **feat(observability): coding lineage, skill provenance, mutation and proof evidence**
   - Record hashes/IDs/evidence, not secrets or unnecessary source payloads.

10. **docs(test): Coding Agent contract and end-to-end acceptance suite**
